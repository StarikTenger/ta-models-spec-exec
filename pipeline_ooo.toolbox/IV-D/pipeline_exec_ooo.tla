...
Init == /\ currCycle = 0
        ...
        /\ _COM = [ i \in (1..superscal) |-> [ PC |-> empty ] ]
         \*/\ IFBus = FALSE
        /\ graph = [ nodes |-> [ i \in (1..ProgLen) |-> [ j \in {"IFacq", "IFrel", "FUacq", "FUrel", "COM"} |-> 0 ] ],
...
ProgressGraph == graph' = [ graph EXCEPT
                            !.nodes = [ i \in 1..Len(graph.nodes) |->
                                            [ IFacq |-> IF Len(prog.exec) >= i /\ \E k \in (1..superscal): _IF[k].PC = prog.exec[i].PC /\ _IF[k].currLat = 1 THEN currCycle ELSE graph.nodes[i].IFacq,
                                              IFrel |-> IF Len(prog.exec) >= i /\ \E k \in (1..superscal): _IF[k].PC = prog.exec[i].PC /\ _IF'[k].PC /= prog.exec[i].PC THEN currCycle+1 ELSE graph.nodes[i].IFrel,
                                              FUacq |-> IF Len(prog.exec) >= i /\ \E k \in (1..N_FU): _FU[k].PC = prog.exec[i].PC /\ _FU[k].currLat = 1 THEN currCycle ELSE graph.nodes[i].FUacq,
                                              FUrel |-> IF Len(prog.exec) >= i /\ \E k \in (1..N_FU): _FU[k].PC = prog.exec[i].PC /\ _FU'[k].PC /= prog.exec[i].PC THEN currCycle+1 ELSE graph.nodes[i].FUrel,
                                              COM |-> IF Len(prog.exec) >= i /\ \E k \in (1..superscal): _COM[k].PC = prog.exec[i].PC THEN currCycle ELSE graph.nodes[i].COM ] ],
                            !.edges = @ \union (UNION ({ [ type : {"RS"}, source : {_FU[i].PC.pc}\{0}, dest : {x.pc : x \in _RS[i] \cup {xx \in UNION({_RS[j] : j \in 1..N_FU}) : i \in xx.type}}] : i \in 1..N_FU }))\* \ { [x EXCEPT !.type = "RS"] : x \in {x \in @ : x.type = "D"} })
...
RAWDep == \A i \in 1..ProgLen: \A d \in Program[i].dep: d < Program[i].pc

...
ProgressGraph == graph' = [ graph EXCEPT
                            ...
                            !.edges = @ \union (UNION ({ [ type : {"RS"}, source : {_FU[i].PC.pc}\{0}, dest : {x.pc : x \in _RS[i]}] : i \in 1..N_FU }))
                                                                            \* \cup {xx \in UNION({_RS[j] : j \in 1..N_FU}) : i \in xx.type}
                                                                            \* \ { [x EXCEPT !.type = "RS"] : x \in {x \in @ : x.type = "D"} })

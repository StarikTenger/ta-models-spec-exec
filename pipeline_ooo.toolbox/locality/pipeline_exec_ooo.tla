...
NxtFU(i) == IF NxtFUBusy(i)
            ...
            ELSE
                ...
                \* At most one instruction issued per cycle in each FU.
...
\* Dispatch (in-order)
\* TODO: RS full => structural hazard => stalling in ID and IF
...
nxtCOM(s) ==  \* The set of (indexes of) instructions *potentially* ready for in-order commit (ROB)
              ...
              LET ready(DoneCOM) == (Exec \intersect readyInOrder(DoneCOM)) \ DoneCOM IN
              \*LET minInstr(min) == IF min /= 0 THEN CHOOSE i \in 1..Len(prog.exec): prog.exec[i].PC.pc = min ELSE 0 IN
              IF s = 1
              THEN
                \* The (index of the) oldest instruction among the instructions already ready for in-order commit
                Min(ready(Done))
              ELSE
                \* The set of (indexes of) the older instructions ready in the SAME cycle
                LET PrevNxt == { nxtCOM(j): j \in 1..s-1 } IN
                \* The next oldest instruction among the instructions ready for in-order commit, taking into account parallel commit
                Min(ready(Done \union PrevNxt))
...
\* Last modified Thu Nov 26 19:54:17 CET 2020 by benjaminbinder

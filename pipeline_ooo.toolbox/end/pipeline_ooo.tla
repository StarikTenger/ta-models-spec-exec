...
VARIABLES commonPre, locWorst, loct \*, globt
...
Spec == Init /\ [][Next]_<< vars, vars2, commonPre, locWorst, loct >>

-----------------------------------------------------------------------------
NotCompleted == \E i \in 1..2: Len(progs[i].rest) > 0 \/ Cardinality(Exec1!Done) < Len(Program) \/ Cardinality(Exec2!Done) < Len(Program)

-----------------------------------------------------------------------------
ProgDone(n) == \A l \in 1..2: \E i \in 1..Len(progs[l].exec): progs[l].exec[i].PC.pc = Program[n].pc /\ progs[l].exec[i].done
...
NoTASteps == \A k \in 1..Len(Program)-1: \A n \in k+1..Len(Program):
             IF /\ ProgDone(n)
                /\ StepHeight(1,k) < StepHeight(2,k)
...
NoTAInter == \A k \in 1..Len(Program)-1: \A n \in k+1..Len(Program):
            IF /\ ProgDone(n)
...
NoTALoc == \* \A n \in 1..Len(Program): \A k \in 1..n: \* It should actually be the execution of *some* sequence in the program...

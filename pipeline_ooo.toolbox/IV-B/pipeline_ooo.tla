...
DomI(i) == IF varIF
           THEN IF modeLen > 0
                THEN IF i%2 = 1 THEN BOOLEAN ELSE { FALSE }
                ELSE Program[i].imiss
           ELSE { FALSE }
DomFU(i) == prog.rest[i].type
DomFULat(i) == prog.rest[i].lat
DomDep(i) == {{}} \union {{j} : j \in i-3..i-1} \*SUBSET(1..i-1)
...
Next == /\ Exec1!Next
        ...
        /\ commonPre' =
            ...
            /\ \A ex \in 1..2: \A k \in 1..superscal: \E kk \in 1..superscal:
                ...
                /\ IFs[ex][k]'.PC = IFs[ex][k].PC /\ IFs[ex][k].PC /= empty => IFs[3-ex][kk]'.PC = IFs[3-ex][kk].PC
            ...
            /\ \A ex \in 1..2: \A k \in 1..N_FU: \E kk \in 1..N_FU:
                ...
                /\ FUs[ex][k]'.PC = FUs[ex][k].PC /\ FUs[ex][k].PC /= empty => FUs[3-ex][kk]'.PC = FUs[3-ex][kk].PC
        /\ locWorst' = [ i \in 1..2 |->
...
ProgDone(n) == \A l \in 1..2: \E i \in 1..Len(progs[l].exec):
                progs[l].exec[i].PC.pc = n /\ progs[l].exec[i].done
...
\* Component occupation (parallel inversion)

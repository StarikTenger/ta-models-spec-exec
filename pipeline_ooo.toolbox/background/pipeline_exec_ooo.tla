...
VARIABLES currCycle, prog, _IF, _ID, _RS, _FU, _COM, iMiss, FU, FULat, graph \*, IFBus
-----------------------------------------------------------------------------
...
Init == /\ currCycle = 0
        ...
        /\ _COM = [ i \in (1..superscal) |-> [ PC |-> empty ] ]
         \*/\ IFBus = FALSE
        /\ graph = [ nodes |-> [ i \in (1..Len(prog.rest)) |-> [ j \in {"IF1", "IF2", "FU1", "FU2", "COM"} |-> 0 ] ],
                     edges |-> UNION { [ type : {"D"}, source : prog.rest[i].dep, dest : {i} ] : i \in DOMAIN prog.rest } ]
-----------------------------------------------------------------------------
...
NxtFU(i) == IF NxtFUBusy(i)
            THEN empty
            ELSE
                \* Older instruction (in program order) among the set of the waiting instructions in the corresponding RS or still in ID,
                ...
                IF minReady = 0 THEN empty ELSE CHOOSE x \in _RS[i] \union FURouting(i): x.pc = minReady
...
ProgressFU == LET FUp(i) == IF ~NxtFUBusy(i)
                            THEN
                                ...
                                ELSE [ PC |-> NxtFU(i), baseLat |-> FULat[NxtFU(i).pc], currLat |-> 1, usage |-> _FU[i].usage+1 ]
...
\* Superscalar commit: the oldest instruction (min. address) is selected among the ready instructions for each iteration of the multiple commit
ProgressCOM == LET nxtPc(i) == IF nxtCOM(i) /= {} THEN prog.exec[Min(nxtCOM(i))].PC ELSE empty IN
...
ProgressProg == LET restp == IF ~NxtIFBusy THEN rest_instr(prog.rest, superscal) ELSE prog.rest IN
                ...
                LET execp == [ i \in 1..Len(nwExec) |-> 
                                ...
                                LET comTimep == IF \E k \in (1..superscal): _COM[k] /= empty /\ _COM[k].PC = instr.PC
                                                THEN currCycle
                                                ELSE instr.comTime IN
                                LET exStartTimep == IF \E k \in (1..N_FU): NxtFU(k) /= empty /\ NxtFU(k) = instr.PC
                                                    THEN currCycle
                                                    ELSE instr.exStartTime IN
                                [ instr EXCEPT !.executed = executedp, !.done = donep, !.exStartTime = exStartTimep, !.comTime = comTimep ]
                             ]
                IN prog' = [ rest |-> restp, exec |-> execp ]
-----------------------------------------------------------------------------
     
ProgressGraph == graph' = [ graph EXCEPT
                            !.nodes = [ i \in 1..Len(graph.nodes) |->
                                            [ IF1 |-> IF Len(prog.exec) >= i /\ \E k \in (1..superscal): _IF[k].PC = prog.exec[i].PC /\ _IF[k].currLat = 1 THEN currCycle ELSE graph.nodes[i].IF1,
                                              IF2 |-> IF Len(prog.exec) >= i /\ \E k \in (1..superscal): _IF[k].PC = prog.exec[i].PC /\ _IF'[k].PC /= prog.exec[i].PC THEN currCycle+1 ELSE graph.nodes[i].IF2,
                                              FU1 |-> IF Len(prog.exec) >= i /\ \E k \in (1..N_FU): _FU[k].PC = prog.exec[i].PC /\ _FU[k].currLat = 1 THEN currCycle ELSE graph.nodes[i].FU1,
                                              FU2 |-> IF Len(prog.exec) >= i /\ \E k \in (1..N_FU): _FU[k].PC = prog.exec[i].PC /\ _FU'[k].PC /= prog.exec[i].PC THEN currCycle+1 ELSE graph.nodes[i].FU2,
                                              COM |-> IF Len(prog.exec) >= i /\ \E k \in (1..superscal): _COM[k].PC = prog.exec[i].PC THEN currCycle ELSE graph.nodes[i].COM ] ],
                            !.edges = @ \union (UNION ({ [ type : {"RS"}, source : {_FU[i].PC.pc}\{0}, dest : {x.pc : x \in _RS[i]} ] : i \in 1..N_FU }))\* \ { [x EXCEPT !.type = "RS"] : x \in {x \in @ : x.type = "D"} })
                                        \union { [ type |-> "ROB", source |-> _COM'[superscal].PC.pc, dest |-> i ] : i \in UNION({nxtCOM(j) : j \in 1..superscal+1}) \ {_COM'[j].PC.pc : j \in 1..superscal} }
                          ]
                 
Progress == ProgressIF /\ ProgressID /\ ProgressRS /\ ProgressFU /\ ProgressCOM /\ ProgressProg /\ ProgressGraph
...
-----------------------------------------------------------------------------
Spec == Init /\ [][Next]_<< prog, pipe_stages, currCycle, iMiss, FU, FULat, graph >>
-----------------------------------------------------------------------------
...
\* Last modified Tue Apr 20 21:46:04 CEST 2021 by benjaminbinder

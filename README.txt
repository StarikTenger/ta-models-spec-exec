...
Detailed content:
	...
	- lib/TAExpl.class: the Java class that overrides the TLA+ module (WARNING: the paths of the py files need to be adapted and the class compiled again).
	- lib/TAExpl.java: the Java library for TLC (see the line above).
...
How to run with the Toolbox:
	...
	2) Make the necessary changes in the Java source (see warning above) and compile the Java class.
	3) Be prepared to add the spec into the toolbox (File menu > Open spec > Add new spec). Then supply the root-module file, i.e, "pipeline_ooo.tla" in the current repository. Ignore the warning about an overwriting of the file and confirm.
	4) Add the derived Java file as a library in the TLA+ toolbox (Menu > Preferences > TLA+ Preferences and add the lib directory mentioned above into the TLA+ library path locations).
	5) Feel free to modify the BUFF_LEN and N_INSTR variables (see warning above).
	6) Execute the shell script by providing an (or several) input C file(s).
	7) Create a TLA+ model by providing the values of the parameters supplied by the shell script (and the value of BUFF_LEN for the ROB size), as well as values for the other, free parameters (e.g., latencies).
